...
import { ArrowLeft, MapPin, Wallet, CreditCard, Banknote, ChevronDown, ChevronUp, Loader2, Info, X, Search, LocateFixed, Check, ShoppingBag, Plus, Minus, Trash2, Home, Briefcase, ReceiptText } from "lucide-react";
...
export default function Cart() {
  ...
  const [activeTab, setActiveTab] = useState<"popular" | "beverages" | "desserts" | "sides">("popular");
  ...
  useEffect(() => {
    const calculateDistance = async () => {
      ...
      const activeAddress = savedAddresses.find((a: any) => a.id === selectedAddressId) || null;
      ...
      if (stallCoords && targetLat && targetLng) {
        ...
        try {
          ...
          if (data.status === 'success' && data.data && data.data.distanceKm >= 4.0) {
            newMarkup = 20;
          }
        } catch (e) {
        ...
        if (cart.items.length > 0) {
          ...
          if (hasMismatchedMarkup) {
            import('react-hot-toast').then(({ default: toast }) => {
              toast.error("Location changed. Cart cleared due to new pricing. Please re-add items.", { id: 'markup_mismatch' });
            });
            clearCart();
            return;
          }
  ...
  const displayedSuggestedItems = suggestedItems.length > 0 ? suggestedItems : (
    cart.stallId === 'rest_2' ? [
      { id: 's1', name: "Whopper", price: 149, img: "/categories/burger.png", isVeg: false },
      { id: 's2', name: "Fries (M)", price: 109, img: "/categories/burger.png", isVeg: true }
    ] : cart.stallId === 'rest_1' ? [
      { id: 's1', name: "Crispy Paneer Patty Burger", price: 114, img: "/categories/burger.png", isVeg: true },
      { id: 's2', name: "Peri Peri Fries", price: 105, img: "/categories/burger.png", isVeg: true },
      { id: 's3', name: "Veggie Sandwich", price: 175, img: "/categories/sandwich.png", isVeg: true }
    ] : cart.stallId === 'meals_99_stall' ? [
      { id: 's3', name: "Chole Bhature", price: 89, img: "/categories/burger.png", isVeg: true },
      { id: 's4', name: "Rajma Chawal", price: 99, img: "/categories/pizza.png", isVeg: true }
    ] : [
      { id: 's1', name: "Sprite", price: 103, img: "/categories/all.png", isVeg: true },
      { id: 's2', name: "Veg Pizza...", price: 55, img: "/categories/pizza.png", isVeg: true },
      { id: 's3', name: "Fries (Large)", price: 125, img: "/categories/burger.png", isVeg: true },
      { id: 's4', name: "Piri Piri Spice Mix", price: 23, img: "/categories/all.png", isVeg: true }
    ]
    ...
    return (
      <div className="min-h-screen bg-[#F5F6F8] flex flex-col items-center justify-center px-4">
        <div className="w-full max-w-sm flex flex-col items-center justify-center p-8 bg-white rounded-[24px] shadow-sm border border-gray-100 text-center">
          ...
          <h2 className="text-[20px] font-bold text-gray-800 mb-2">Your cart is empty</h2>
          <p className="text-[14px] text-gray-500 mb-8 font-medium">Looks like you haven't added anything to your cart yet.</p>
          <button onClick={() => router.push('/')} className="w-full bg-[#00A14F] text-white font-bold text-[15px] py-4 rounded-full shadow-[0_4px_12px_rgba(0,161,79,0.3)] hover:bg-[#009146] transition-colors">
            Explore Menu
          </button>
        </div>
      </div>
      ...
      <div className="app-scroll-container bg-[#F5F6F8] pb-[100px] font-body relative h-[100vh] overflow-y-auto hide-scrollbar">
        ...
        <div className="bg-[#00A14F] px-4 pt-4 pb-[80px] sticky top-0 z-0 overflow-hidden">
          <div className="flex items-center gap-3 relative z-10">
            <button onClick={() => router.back()} className="w-8 h-8 flex items-center justify-center rounded-full text-white hover:bg-white/10 transition-colors">
              <ArrowLeft size={24} />
            </button>
            <h1 className="font-bold text-[16px] text-white flex-1 tracking-wide">{cart.stallName}</h1>
          </div>
          <div className="absolute right-[-10px] top-2 opacity-10 pointer-events-none z-0 flex flex-col items-center transform -rotate-12">
            <svg width="120" height="30" viewBox="0 0 120 30">
              ...
              <text className="font-bold text-[14px] tracking-widest" fill="white">
                 <textPath href="#curve" startOffset="50%" textAnchor="middle">EVERYDAY</textPath>
              </text>
            </svg>
            <span className="font-black text-[18px] text-white leading-none tracking-tight -mt-1">LOWEST PRICE</span>
          </div>
        </div>
        ...
        <div className="relative z-10 -mt-[60px] bg-[#F5F6F8] rounded-t-[24px] min-h-screen px-3 pt-3 flex flex-col gap-3 pb-24">
          ...
          <div className="bg-white rounded-[20px] p-4 shadow-sm border border-gray-100">
            {cart.items.map((item, idx) => (
              <div key={item.id || idx} className="flex flex-col mb-4 last:mb-0">
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-2 max-w-[55%]">
                    <div className="mt-[3px] shrink-0">{item.isVeg ? <VegIcon /> : <NonVegIcon />}</div>
                    <h3 className="font-bold text-[14px] text-gray-800 leading-tight">{item.name}</h3>
                  </div>
                  ...
                  <div className="flex items-start gap-3 shrink-0">
                    ...
                    <div className="flex flex-col items-end w-[40px]">
                      <span className="text-[12px] text-gray-400 line-through font-medium mb-0.5">â‚¹{Math.round(item.price * 1.15)}</span>
                      <span className="text-[14px] font-bold text-gray-800">â‚¹{item.price}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 mt-4 overflow-x-auto hide-scrollbar pb-1">
              <button onClick={() => router.push('/')} className="flex items-center gap-1.5 border border-gray-200 rounded-full px-3 py-1.5 text-[12px] font-medium text-gray-500 shrink-0 hover:bg-gray-50">
              ...
              <button className="flex items-center gap-1.5 border border-gray-200 rounded-full px-3 py-1.5 text-[12px] font-medium text-gray-500 shrink-0 hover:bg-gray-50">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg> 
                Cooking requests
              </button>
          ...
          <div className="bg-white rounded-[24px] p-4 shadow-sm border border-gray-100 overflow-hidden">
            <h3 className="text-[12px] font-bold text-gray-500 uppercase tracking-widest mb-4">COMPLETE YOUR MEAL</h3>
            ...
            <div className="flex overflow-x-auto hide-scrollbar gap-3 pb-2 snap-x pt-2">
               {displayedSuggestedItems.map((sitem) => (
                 <div key={sitem.id} className="flex flex-col shrink-0 w-[105px] snap-start">
                    <div className="w-[105px] h-[105px] bg-white rounded-[12px] mb-2 relative overflow-visible border border-gray-200 shadow-sm flex items-center justify-center p-2">
                       <Image src={sitem.img || sitem.image} alt={sitem.name} fill className="object-cover p-2 rounded-[12px]" />
                       <button 
                         onClick={() => updateQuantity(cart.stallId || '1', cart.stallName || 'Restaurant', { id: sitem.id, name: sitem.name, price: sitem.price, isVeg: sitem.isVeg }, 1)}
                    ...
                    <div className="flex items-start gap-1">
                      <div className="mt-[2px] shrink-0">{sitem.isVeg ? <VegIcon /> : <NonVegIcon />}</div>
                      <h4 className="font-medium text-[13px] text-gray-800 leading-tight line-clamp-2 w-full">{sitem.name}</h4>
                    </div>
                    <span className="font-bold text-[13px] text-gray-800 mt-1 pl-[15px]">â‚¹{sitem.price}</span>
                 </div>
               ))}
            </div>
          </div>
          ...
          <div className="bg-white rounded-[20px] p-4 shadow-sm border border-gray-100">
             <div className="flex items-center gap-1 bg-[#F5F6F8] rounded-full p-1 mb-5">
                <button className="flex-1 bg-white shadow-[0_2px_8px_rgba(0,0,0,0.04)] rounded-full py-2.5 flex items-center justify-center gap-1.5 text-[13px] font-bold text-green-800">
                  Delivery Modes <span className="bg-[#00A14F] text-white text-[9px] px-1.5 py-0.5 rounded-sm tracking-wider">NEW</span>
                </button>
             ...
             <div className="flex flex-col gap-4">
                <label className="flex items-start gap-3 cursor-pointer group">
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 transition-colors ${deliveryMode === 'quick' ? 'border-[#FF007F]' : 'border-gray-300'}`}>
                    {deliveryMode === 'quick' && <div className="w-2.5 h-2.5 bg-[#FF007F] rounded-full"></div>}
                  </div>
                  <input type="radio" name="deliveryMode" className="hidden" checked={deliveryMode === 'quick'} onChange={() => setDeliveryMode('quick')} />
                  <div className="flex flex-col flex-1">
                    <div className="flex justify-between items-center w-full mb-0.5">
                       <span className={`font-medium text-[15px] flex items-center gap-1 transition-colors ${deliveryMode === 'quick' ? 'text-gray-400' : 'text-gray-300'}`}>Quick <span className="text-[#FF007F]">âš¡</span></span>
                       <span className={`font-medium text-[14px] transition-colors ${deliveryMode === 'quick' ? 'text-gray-500' : 'text-gray-300'}`}>+â‚¹7</span>
                    </div>
                    <span className="text-[12px] text-gray-300 font-medium">Add address to check delivery time</span>
                  </div>
                </label>
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 transition-colors ${deliveryMode === 'basic' ? 'border-[#FF007F]' : 'border-gray-300'}`}>
                    {deliveryMode === 'basic' && <div className="w-2.5 h-2.5 bg-[#FF007F] rounded-full"></div>}
                  </div>
                  <input type="radio" name="deliveryMode" className="hidden" checked={deliveryMode === 'basic'} onChange={() => setDeliveryMode('basic')} />
                  <div className="flex flex-col flex-1">
                    <div className="flex justify-between items-center w-full mb-0.5">
                       <span className={`font-medium text-[15px] transition-colors ${deliveryMode === 'basic' ? 'text-gray-400' : 'text-gray-300'}`}>Basic</span>
                    </div>
                    <span className="text-[12px] text-gray-300 font-medium">Add address to check delivery time</span>
                  </div>
                </label>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
             <div className="flex items-center gap-3">
               <div className="w-8 h-8 bg-green-700 text-white rounded-[10px] flex items-center justify-center">
                 <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7" stroke="white" strokeWidth="2" strokeLinecap="round"/></svg>
               </div>
               <span className="font-bold text-[14px] text-gray-800">Payment offers & more</span>
             </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center justify-between">
             <div className="flex items-center gap-3">
               ...
               <span className="font-bold text-[14px] text-gray-800">â‚¹9 saved with 'Items at â‚¹49'</span>
             </div>
             <span className="text-green-700 text-[13px] font-bold flex items-center gap-0.5"><Check size={14} strokeWidth={3} /> Applied</span>
          </div>

          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-4 flex flex-col gap-4">
            <h3 className="font-bold text-[15px] text-gray-800">Select Payment Method</h3>
            
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${paymentMethod === 'upi' ? 'border-[#FF007F]' : 'border-gray-300'}`}>
                {paymentMethod === 'upi' && <div className="w-2.5 h-2.5 bg-[#FF007F] rounded-full"></div>}
              </div>
              ...
              <div className="flex items-center gap-2 flex-1">
                ...
                <span className={`font-medium text-[15px] transition-colors ${paymentMethod === 'upi' ? 'text-gray-800' : 'text-gray-500'}`}>Pay Online (Razorpay)</span>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${paymentMethod === 'cod' ? 'border-[#FF007F]' : 'border-gray-300'}`}>
                {paymentMethod === 'cod' && <div className="w-2.5 h-2.5 bg-[#FF007F] rounded-full"></div>}
              </div>
              ...
              <div className="flex items-center gap-2 flex-1">
                ...
                <span className={`font-medium text-[15px] transition-colors ${paymentMethod === 'cod' ? 'text-gray-800' : 'text-gray-500'}`}>Cash on Delivery</span>
              </div>
            </label>
          </div>

          {/* Bill Summary */}
          <div className={`bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden transition-all duration-300 ${isBillExpanded ? 'pb-0' : ''}`}>
             <div 
               ...
               <div className="flex items-center gap-3">
                 ...
                 <div className="flex flex-col">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-[15px] text-gray-800">To Pay</span>
                      <span className="text-[14px] text-gray-400 line-through font-medium">â‚¹{finalTotal + 9}</span>
                      <span className="font-bold text-[15px] text-gray-800">â‚¹{finalTotal}</span>
                    </div>
                    <span className="text-green-600 text-[13px] font-bold mt-0.5">â‚¹9 saved on the total!</span>
                 </div>
               </div>
               {isBillExpanded ? <ChevronUp size={22} className="text-gray-800" strokeWidth={2.5} /> : <ChevronDown size={22} className="text-gray-800" strokeWidth={2.5} />}
             </div>
               ...
               <div className="flex flex-col w-full animate-in slide-in-from-top-2 duration-200">
                 ...
                 <div className="px-4 py-4 flex flex-col gap-4">
                   <div className="flex items-center justify-between text-[14px]">
                     ...
                     <div className="flex items-center gap-2">
                        <span className="text-gray-500 line-through font-medium">â‚¹{itemTotal + 9}</span>
                        <span className="text-[#00A14F] font-bold">â‚¹{itemTotal}</span>
                     </div>
                   </div>
                   ...
                   <div className="flex items-center justify-between text-[14px]">
                     <span className="text-gray-500 font-medium border-b border-dashed border-gray-400 pb-[1px] leading-none">Delivery Fee | 2.6 kms</span>
                     <span className="text-gray-600 font-medium">â‚¹{deliveryFee}</span>
                   </div>
                   
                   <div className="flex items-center justify-between text-[14px]">
                     <span className="text-gray-500 font-medium border-b border-dashed border-gray-400 pb-[1px] leading-none">Government Taxes</span>
                     <span className="text-gray-600 font-medium">â‚¹{GST.toFixed(2)}</span>
                   </div>
                 ...
                 <div className="w-full bg-gradient-to-r from-[#FFF0F5] to-white px-4 py-3 relative overflow-hidden flex items-center min-h-[70px]">
                    <div className="w-[80%] relative z-10">
                      <p className="text-[#6D1B4B] text-[13px] font-medium leading-[1.4]">No platform fee, No packaging fee &<br/>Lowest item prices</p>
                    </div>
                    
                    {/* Pink SVG Stamp */}
                    <div className="absolute right-[-15px] bottom-[-15px] w-[100px] h-[100px] z-0 opacity-90 transform -rotate-[15deg]">
                       <svg viewBox="0 0 100 100" className="w-full h-full text-[#FF007F]">
                         ...
                         <defs>
                           <path id="curvePink" d="M 22,50 A 28,28 0 0,1 78,50" fill="none" />
                         </defs>
                         <text fill="currentColor" fontSize="10" fontWeight="900" letterSpacing="0.5" style={{fontFamily: 'Arial, sans-serif'}}>
                           <textPath href="#curvePink" startOffset="50%" textAnchor="middle">EVERYDAY</textPath>
                         </text>
                         <path d="M 5,42 L 95,42 L 91,52 L 95,62 L 5,62 L 9,52 Z" fill="currentColor" />
                         <text x="50" y="55" fill="white" fontSize="10" fontWeight="900" letterSpacing="0" textAnchor="middle" style={{fontFamily: 'Arial, sans-serif'}}>LOWEST PRICE</text>
                       </svg>
                    </div>
                 </div>
               </div>
          ...
          <div className="px-2 mt-2 mb-10">
            <h4 className="text-[12px] font-bold text-gray-500 mb-1">Cancellation policy:</h4>
        ...
        <div className="fixed bottom-0 left-0 w-full bg-white rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.08)] p-4 pt-5 pb-6 z-50 transition-transform">
          <div className="flex items-center justify-between mb-3 px-1 cursor-pointer" onClick={() => setIsAddressListOpen(true)}>
            <div className="flex flex-col">
              <div className="flex items-center gap-1.5">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#00A14F" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 2L11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/></svg>
                <h3 className="font-black text-[14px] text-gray-800 tracking-tight">{savedAddresses.find((a: any) => a.id === selectedAddressId)?.tag || 'Delivery Address'}</h3>
              </div>
              <p className="text-[12px] text-gray-500 mt-0.5 line-clamp-1 ml-5">
                {savedAddresses.find((a: any) => a.id === selectedAddressId)?.fullAddress || 'Select delivery address'}
              </p>
            </div>
            <span className="text-[12px] font-bold text-[#00A14F] bg-green-50 px-2 py-1 rounded-md">CHANGE</span>
          </div>
          ...
          >
            {isPlacingOrder ? <Loader2 className="animate-spin" size={20} /> : null}
        ...
        {isAddressListOpen && (
          ...
          >
            <div className="p-4 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white z-10 shadow-sm">
              <h2 className="text-[18px] font-black text-gray-800 tracking-tight">Select Delivery Address</h2>
              <button onClick={() => setIsAddressListOpen(false)} className="p-2 bg-gray-50 rounded-full">
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 pb-24">
              {savedAddresses.map((addr: any) => (
                ...
                >
                  <div className="mt-0.5"><MapPin size={20} className={selectedAddressId === addr.id ? 'text-[#00A14F]' : 'text-gray-400'} /></div>
                  <div className="flex flex-col w-full">
                    <div className="flex items-center justify-between w-full">
                      <h4 className="font-bold text-gray-800 text-[15px]">{addr.tag || 'Address'}</h4>
                      {selectedAddressId === addr.id && <Check size={18} className="text-[#00A14F]" strokeWidth={3} />}
                    </div>
                    <p className="text-gray-500 text-[13px] leading-relaxed mt-1 line-clamp-2">{addr.fullAddress}</p>
                  </div>
                </div>
              ))}
            ...
            <div className="flex-1 relative">
              ...
              <div className="absolute inset-0 w-full h-full bg-[#f5f5f5]">
                {!mapboxToken ? (
                  ...
                  <LocationPickerMap apiKey={mapboxToken} initialLocation={{lat: mapLat, lng: mapLng}} onLocationSelect={(lat, lng) => handleMapDragEnd(lat, lng)} />
                )}
