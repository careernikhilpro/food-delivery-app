                  ...
                  <div key={idx} className="flex justify-between items-center p-3 border border-gray-100 rounded-xl hover:border-gray-200 transition-colors">
                    ...
                    {qty > 0 ? (
                      <div className="flex items-center justify-between w-[80px] h-[32px] bg-green-50 border border-green-200 text-green-700 font-bold text-[14px] rounded-lg">
                        <button onClick={() => updateQuantity(variantModal.stallId, variantModal.stallName, { id: vId, name: `${variantModal.item.name} - ${v.name}`, price: vPrice, image: variantModal.item.image_url, isVeg: variantModal.item.is_veg, is_free_delivery: variantModal.item.is_free_delivery, free_delivery_min_amount: variantModal.item.free_delivery_min_amount, free_delivery_max_km: variantModal.item.free_delivery_max_km, coupon_applicable: variantModal.item.coupon_applicable }, -1)} className="w-1/3 flex justify-center py-1"><Minus size={14} /></button>
                        <span>{qty}</span>
                        <button onClick={() => updateQuantity(variantModal.stallId, variantModal.stallName, { id: vId, name: `${variantModal.item.name} - ${v.name}`, price: vPrice, image: variantModal.item.image_url, isVeg: variantModal.item.is_veg, is_free_delivery: variantModal.item.is_free_delivery, free_delivery_min_amount: variantModal.item.free_delivery_min_amount, free_delivery_max_km: variantModal.item.free_delivery_max_km, coupon_applicable: variantModal.item.coupon_applicable }, 1)} className="w-1/3 flex justify-center py-1"><Plus size={14} /></button>
                      </div>
                    ) : (
                      <button 
                        onClick={() => updateQuantity(variantModal.stallId, variantModal.stallName, { id: vId, name: `${variantModal.item.name} - ${v.name}`, price: vPrice, image: variantModal.item.image_url, isVeg: variantModal.item.is_veg, is_free_delivery: variantModal.item.is_free_delivery, free_delivery_min_amount: variantModal.item.free_delivery_min_amount, free_delivery_max_km: variantModal.item.free_delivery_max_km, coupon_applicable: variantModal.item.coupon_applicable }, 1)}
