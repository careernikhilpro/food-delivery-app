...
export default function Search() {
  ...
  return (
    <div className="min-h-screen bg-bg-main font-body pb-24 app-scroll-container">
      {/* Header & Search Bar */}
      <div className="sticky top-0 z-30 bg-bg-main/80 backdrop-blur-2xl pt-4 pb-5 px-4 sm:px-6 xl:px-8 flex items-center gap-3 border-b border-gray-100/50 shadow-[0_4px_20px_rgb(0,0,0,0.02)]">
        <button onClick={() => router.back()} className="w-11 h-11 flex items-center justify-center rounded-full bg-white hover:bg-gray-50 border border-gray-100 transition-all duration-300 shadow-[0_4px_10px_rgb(0,0,0,0.04)] hover:shadow-[0_4px_15px_rgb(0,0,0,0.08)] hover:-translate-y-0.5 active:scale-95 shrink-0">
          <ArrowLeft size={20} className="text-gray-700" />
        </button>
        
        <div className="flex-1 relative group">
          <div className="absolute inset-0 bg-gradient-to-r from-[#FF5722]/20 to-orange-400/20 rounded-[20px] blur-xl opacity-0 group-focus-within:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
          <input 
            ref={inputRef}
            type="text" 
            value={query}
            ...
            className="w-full relative z-10 bg-white border border-gray-100 rounded-[20px] py-4 pl-5 pr-14 text-[14px] font-bold text-gray-800 placeholder-gray-400 outline-none focus:border-[#FF5722]/30 focus:ring-4 focus:ring-[#FF5722]/10 transition-all duration-300 shadow-[0_8px_30px_rgb(0,0,0,0.05)] focus:shadow-[0_8px_30px_rgb(255,87,34,0.1)]"
          />
          {query ? (
            <button onClick={() => setQuery("")} className="absolute right-4 top-1/2 -translate-y-1/2 p-2 text-gray-400 hover:text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-full transition-colors z-20">
              <X size={16} strokeWidth={3} />
            </button>
          ) : (
            <div className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 flex items-center pr-1 border-l border-gray-100 pl-2 z-20">
              <button onClick={startVoiceSearch} className="p-2.5 text-[#FF5722] hover:bg-[#FF5722]/10 rounded-full transition-colors active:scale-95">
      ...
      <div className="px-4 sm:px-6 xl:px-8 mt-8 max-w-3xl mx-auto">
        {viewState === 'empty' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            {recentSearches.length > 0 && (
              <div className="mb-10">
                <h3 className="font-heading font-black text-[15px] uppercase tracking-wider text-gray-800 mb-5 flex items-center gap-2.5">
                  <div className="bg-orange-50 p-1.5 rounded-lg border border-orange-100/50">
                    <Clock size={16} className="text-[#FF5722]" strokeWidth={2.5} />
                  </div>
                  Recent Searches
                </h3>
                <div className="flex flex-wrap gap-3">
                  {recentSearches.map(s => (
                    <button key={s} onClick={() => executeSearch(s, 'dish')} className="px-4.5 py-2.5 rounded-[16px] border border-gray-100 text-[13px] font-bold text-gray-600 hover:border-[#FF5722]/30 hover:bg-[#FF5722]/5 hover:text-[#FF5722] transition-all duration-300 bg-white shadow-[0_4px_15px_rgb(0,0,0,0.03)] hover:shadow-[0_4px_15px_rgb(255,87,34,0.12)] hover:-translate-y-0.5 flex items-center gap-2 group">
                      <SearchIcon size={14} className="text-gray-400 group-hover:text-[#FF5722] transition-colors" strokeWidth={2.5} /> {s}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div>
              <h3 className="font-heading font-black text-[15px] uppercase tracking-wider text-gray-800 mb-5 flex items-center gap-2.5">
                <div className="bg-[#FF5722]/10 p-1.5 rounded-lg border border-[#FF5722]/20">
                  <TrendingUp size={16} className="text-[#FF5722]" strokeWidth={2.5} />
                </div>
                Popular Cuisines
              </h3>
              <div className="flex flex-wrap gap-3">
                {popularCuisines.map(c => (
                  <button key={c} onClick={() => executeSearch(c, 'dish')} className="px-5 py-3 rounded-[16px] bg-gradient-to-br from-white to-gray-50/50 border border-gray-100 shadow-[0_4px_15px_rgb(0,0,0,0.04)] text-[13px] font-bold text-gray-700 hover:from-[#FF5722]/5 hover:to-[#FF5722]/10 hover:text-[#FF5722] hover:border-[#FF5722]/30 transition-all duration-300 hover:shadow-[0_6px_20px_rgb(255,87,34,0.15)] hover:-translate-y-0.5 relative overflow-hidden group">
