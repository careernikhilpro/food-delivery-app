...
const regex = /<div className="absolute right-\[0%\] top-\[-10px\] w-\[65%\] h-\[200px\] bg-\[\#FCE38A\] z-0" style={{ clipPath: 'polygon\(40% 0%, 100% 0%, 75% 50%, 85% 100%, 0% 100%, 75% 50%\)' }}><\/div>\s*<div className="relative z-10 flex flex-col max-w-\[55%\] pl-6 mb-0 justify-center h-full">\s*<div className="relative w-\[170px\] h-\[120px\] mt-0">\s*<Image\s*src="\/prices\.png"\s*alt="Prices That Slay Everyday"\s*fill\s*className="object-contain object-left"\s*priority\s*unoptimized\s*\/>\s*<\/div>\s*<Link href="\/lowest-prices">\s*<button className="bg-\[\#FF007F\] text-white font-black text-\[13px\] px-6 py-2\.5 rounded-full w-fit uppercase tracking-wide mt-1 relative z-20 ml-0 hover:scale-105 transition-transform">\s*ORDER NOW\s*<\/button>\s*<\/Link>\s*<\/div>/g;
...
code = code.replace(regex, newBannerContent);
fs.writeFileSync('swaddo-customer-app/src/app/page.tsx', code);
console.log('Fixed banner routes and replaced top banner via REGEX');
