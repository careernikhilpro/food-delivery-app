...
router.patch('/:id/status', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (status === 'heading_to_customer') {
      const expectedPin = String((parseInt(id) * 83) % 10000).padStart(4, '0');
      if (pin !== expectedPin) {
        return res.status(400).json({ message: 'Invalid verification PIN provided.' });
      }
    } else if (status === 'delivered') {
      const expectedPin = String((parseInt(id) * 137) % 10000).padStart(4, '0');
      if (pin !== expectedPin) {
        return res.status(400).json({ message: 'Invalid verification PIN provided.' });
      }
    }
