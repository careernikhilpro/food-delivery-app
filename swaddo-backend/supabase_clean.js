...
async function clean() {
  try {
    ...
    const d1 = await client.query('DELETE FROM menu_items WHERE stall_id !=  RETURNING id', [keepId]);
    console.log('Deleted menu items:', d1.rowCount);
    const d2 = await client.query('DELETE FROM stalls WHERE id !=  RETURNING id', [keepId]);
