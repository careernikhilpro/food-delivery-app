...
async function clean() {
  try {
    ...
    await client.query('DELETE FROM delivery_assignments WHERE order_id IN (SELECT id FROM orders WHERE stall_id != ' + keepId + ')');
    await client.query('DELETE FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE stall_id != ' + keepId + ')');
    await client.query('DELETE FROM orders WHERE stall_id != ' + keepId);
