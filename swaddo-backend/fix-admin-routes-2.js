...
const regex = /const \{ name, description, price, is_veg, is_available, category, variants, prep_time_minutes, discount_percentage, addons \} = req\.body;[\s\S]*?WHERE id = \$11 RETURNING \*`,\s*\[[\s\S]*?itemId\s*\]\s*\);/;
...
if (regex.test(code)) {
  ...
  console.log('Could not match regex for put route');
}
