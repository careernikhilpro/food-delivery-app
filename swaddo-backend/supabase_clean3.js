...
async function clean() {
  try {
    ...
    // Inlining ID to avoid prepared statement issues with pgbouncer
    const d1 = await client.query('DELETE FROM menu_items WHERE stall_id != ' + keepId);
    console.log('Deleted menu items:', d1.rowCount);
    
    // Delete stall categories if they exist (foreign key constraint from stalls)
    // Actually stalls has NO stall_categories table maybe? Let's try to just delete stalls. If it fails, we will see.
